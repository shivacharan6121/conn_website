    ...
    if (isset($_POST['form_type'])) {
        ...
        switch ($form_type) {
            case 'add_part':
                ...
                $check_sql = "SELECT quantity FROM part WHERE Nomenclature = '$part_no'";
                $result = $conn->query($check_sql);

                if ($result->num_rows > 0) {
                    $_SESSION['alert'] = [
                        'type' => 'error',
                        'message' => "Entered part number <strong>$part_no</strong> already exists!"
                    ];
                } else {
            ...
            case 'add_conn':
                ...
                if ($result->num_rows == 0) {
                    $_SESSION['alert'] = [
                        'type' => 'warning',
                        'message' => "Part number <strong>$part_no</strong> does not exist! Please add the part first."
                    ];
                } else {
                    $_SESSION['Nomenclature'] = $part_no;
                    $_SESSION['quantity'] = $add_quantity;
                    $message = "Are you sure you want to add <strong>$add_quantity</strong> connectors to part number <strong>$part_no</strong>?";
                    $show_confirmation = true;
                }
                break;
            ...
            case 'required_conn':
                ...
                if ($result->num_rows > 0) {
                    ...
                    if ($remove_quantity > $current_quantity) {
                        ...
                        $message = "Are you sure you want to remove <strong>$remove_quantity</strong> connectors from part <strong>$part_no</strong>?";
                        $show_confirmation = true;
                    }
                }
                break;
    ...
    if (isset($_POST['confirm'])) {
        ...
        switch ($form_type) {
            ...
            case 'add_conn':
                ...
                $check_sql = "SELECT quantity, availableqty, usedqty FROM part WHERE Nomenclature = '$part_no'";
                $result = $conn->query($check_sql);
                ...
                if ($result->num_rows > 0) {
                    ...
                    $usedqty = $row['usedqty'];
                    $sql = "UPDATE part SET quantity = $new_quantity, availableqty = $availablequty WHERE Nomenclature = '$part_no'";
                    if ($conn->query($sql) === TRUE) {
                        $_SESSION['alert'] = [
                            'type' => 'success',
                            'message' => "Successfully added <strong>$add_quantity</strong> connectors to part number <strong>$part_no</strong> \n\n    Total quantity: <strong>$new_quantity</strong> \n  Used quantity: <strong>$usedqty</strong> \n   Available quantity: <strong>$availablequty</strong>"
                        ];
                    } else {
                        $_SESSION['alert'] = [
                            'type' => 'error',
            ...
            case 'required_conn':
                ...
                if ($result->num_rows > 0) {
                    ...
                    $sql = "UPDATE part SET availableqty = $available_quantity, usedqty = $usedqty WHERE Nomenclature = '$part_no'";
                    if ($conn->query($sql) === TRUE) {
                        $_SESSION['alert'] = [
                            'type' => 'success',
                            'message' => "Removed <strong>$remove_quantity</strong> connectors from part number <strong>$part_no</strong> \n\nAvailable quantity: <strong>$available_quantity</strong>\nUsed quantity: <strong>$usedqty</strong>\nTotal quantity: <strong>$total_quantity</strong>"
                        ];
                    } else {
                        $_SESSION['alert'] = [
                            'type' => 'error',
        ...
        // Clear session data
        unset($_SESSION['form_type'], $_SESSION['part_no'], $_SESSION['quantity'], $_SESSION['add_quantity'], $_SESSION['remove_quantity']);
